...
public class AddressBookAppTest {
    ...
    public void testJUnitGetAndValidateContactNumber(){ 
        String addPhoneNum = "2";
        String ph = addressBook.getAndValidateContactNumber(addPhoneNum);
        assertEquals(addPhoneNum, addressBook.getAndValidateContactNumber(addPhoneNum));
    }
    @Test
   	public void testJUnitGetAndValidateContactName(){ 
       	String contactName = "Shelley";
   		assertEquals(contactName, addressBook.getAndValidateContactName(contactName));
   	}
    @Test
   	public void testJUnitGetAndValidateAddressBookName(){ 
       	String addressBookName = "Book1";
   		assertEquals(addressBookName, addressBook.getAndValidateAddressBookName(addressBookName));
   	}
	...
	public void testAddContactsInAddressBook() {
		...
		assertEquals("Adding 1 more address info to map", count+1, addressBooks.size());
		} 
}
